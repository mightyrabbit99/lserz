...
inline static void ship_s_print(struct ship_s *ship, void (*f)(void *, const char *format, ...), void *arg) {
  ...
  f(arg, "ship tonnage: %d\n", ship->num_of_engines);
  f(arg, "ship engine_ids:");
  for (typeof(ship->num_of_engines) i = 0; i < ship->num_of_engines; i++) f(arg, " %.2x", ship->engine_ids[i]);
  f(arg, "\n");
}
...
inline static int ship_s_filldummy(struct ship_s *ship) {
  ...
  ship->num_of_engines = 4;
  ship->engine_ids = (uint8_t *)malloc(ship->num_of_engines);
  for (typeof(ship->num_of_engines) i = 0; i < ship->num_of_engines; i++) ship->engine_ids[i] = 0x40 + (uint8_t)i;
  return 0;
}
