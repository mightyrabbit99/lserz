...
SZFDECL_STATIC(struct, ship_s, p) {
  ...
  szcv(szc_dtyp_o, p->num_of_engines);
  szcmlcl((void **)&p->engine_ids, p->num_of_engines * sizeof(p->engine_ids[0]));
  szc_foreach(idx, p->num_of_engines) {
    szcv(szc_dtyp_o, p->engine_ids[idx]);
  }
  return 0;
}
